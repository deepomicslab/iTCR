...
# Default feature pairs for TCR analysis
...
def perform_statistical_tests(df_plot: pd.DataFrame, 
    ...
    df_plot : pd.DataFrame
        DataFrame with columns 'Sample_ID' and 'MI_Value'
    unique_samples : List[str]
    ...
    pd.DataFrame
        ...
        Test_Direction_Used, N_Sample1, N_Sample2
    """
...
def create_combined_boxplots(all_boxplot_data: List[Dict], 
    ...
    """
    Create combined boxplot figure showing MCR values across samples for multiple feature pairs.
    
    Parameters:
    -----------
    all_boxplot_data : List[Dict]
        List of dictionaries containing boxplot data for each feature pair
    test_direction : TestDirection
    ...
    for i, data in enumerate(all_boxplot_data):
        ...
        # Create boxplot data
        box_data = [df_plot[df_plot['Sample_ID'] == sample]['MI_Value'].values 
                   for sample in unique_samples]
        ...
        ax.set_ylabel('MCR Value', fontsize=12)
    ...
    if save_dir:
        ...
        combined_filename = f"combined_boxplots.png"
...
def create_combined_heatmaps(all_heatmap_data: List[Dict], 
                           ...
                           save_dir: Optional[str] = None) -> None:
    """
    Create combined heatmap figure showing p-values between samples.
    ...
    for i, data in enumerate(all_heatmap_data):
        ...
        if len(results_df) == 0 or len(unique_samples) < 2:
            ...
            ax.set_title(f'{feature1}, {feature2}', fontsize=14, fontweight='bold')
            continue
    ...
    if save_dir:
        os.makedirs(save_dir, exist_ok=True)
        combined_filename = f"combined_heatmaps.png"
        save_path = os.path.join(save_dir, combined_filename)
        plt.savefig(save_path, bbox_inches='tight', dpi=300)
        print(f"📊 Heatmap saved: {combined_filename}")


def parse_feature_pairs(feature_string: str) -> List[Tuple[str, str]]:
    """
    Parse feature pairs from command line string.
    
    Expected format: "feat1,feat2;feat3,feat4;feat5,feat6"
    
    Parameters:
    -----------
    feature_string : str
        String representation of feature pairs
        
    Returns:
    --------
    List[Tuple[str, str]]
        List of feature pairs as tuples
    """
    try:
        pairs = []
        for pair_str in feature_string.split(';'):
            feat1, feat2 = pair_str.strip().split(',')
            pairs.append((feat1.strip(), feat2.strip()))
        return pairs
    except Exception as e:
        raise ValueError(f"Invalid feature pairs format. Expected 'feat1,feat2;feat3,feat4'. Error: {e}")


def create_mi_analysis(mcr_path: str, 
                      save_dir: Optional[str] = "figures/MI_analysis", 
                      selected_features: Optional[List[Tuple[str, str]]] = None, 
                      test_direction: TestDirection = 'two-sided', 
                      adjust_pvalues: bool = True, 
                      adjust_method: AdjustMethod = 'Bonferroni', 
                      significance_threshold: float = 0.05,
                      no_display: bool = False) -> Dict[Tuple[str, str], pd.DataFrame]:
    """
    Create comprehensive MCR analysis with boxplots and p-value heatmaps.
    
    Parameters:
    -----------
    mcr_path : str
        Path to the pickle file containing MCR data
    save_dir : Optional[str]
        Directory to save figures. If None, figures are not saved.
    selected_features : Optional[List[Tuple[str, str]]]
        List of feature pairs to analyze. If None, uses DEFAULT_FEATURE_PAIRS.
    test_direction : TestDirection
    ...
    Dict[Tuple[str, str], pd.DataFrame]
        Dictionary mapping feature pairs to their statistical test results
    """
    
    # Use default feature pairs if none specified
    if selected_features is None:
        selected_features = DEFAULT_FEATURE_PAIRS
    
    print(f"🔬 Starting MCR Analysis...")
    print(f"   Selected features: {len(selected_features)} pairs")
    ...
    # Load data
    try:
        mi = pd.read_pickle(mcr_path)
        print(f"✅ Successfully loaded MCR data from: {mcr_path}")
    except Exception as e:
        print(f"❌ Error loading MCR data: {str(e)}")
        return {}
    
    first_feature_pair = list(mi.keys())[0]
    first_resample = list(mi[first_feature_pair].keys())[0]
    sample_ids = list(mi[first_feature_pair][first_resample].keys())
    ...
    if len(feature_pairs) == 0:
        print("❌ No matching feature pairs found!")
        return {}
    ...
    for feature1, feature2 in feature_pairs:
        ...
        # Prepare plot data
        plot_data = []
        for resample_idx in mi_data:
            for sample_id in sample_ids:
                if sample_id in mi_data[resample_idx]:
                    plot_data.append({
                        'Sample_ID': sample_id,
                        'MI_Value': mi_data[resample_idx][sample_id]
                    })
        
        df_plot = pd.DataFrame(plot_data)
        if df_plot.empty:
            print(f"     ⚠️  No data found for {feature1} vs {feature2}")
            continue
        ...
        all_results[(feature1, feature2)] = results_df
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Mutual Information (MCR) Analysis Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with default settings
  python MI_display.py --mcr_path data.pkl
  
  # Specify output directory and test direction
  python MI_display.py --mcr_path data.pkl --save_dir results/ --test_direction two-sided
  
  # Use FDR correction and custom threshold
  python MI_display.py --mcr_path data.pkl --adjust_method FDR --significance_threshold 0.01
  
  # Custom feature pairs
  python MI_display.py --mcr_path data.pkl --features "TRAV,TRBV;cdr3A,cdr3B"
  
  # Batch mode (no display)
  python MI_display.py --mcr_path data.pkl --no_display --save_dir batch_results/

Default feature pairs:
  TRAV,TRBV; cdr3A,cdr3B; TRAV,cdr3B; cdr3A,TRBV; TRAJ,TRBJ; cdr3A,TRBJ; TRAJ,cdr3B
        """
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help='Path to the pickle file containing MCR data'
    )
    ...
    parser.add_argument(
        '--save_dir', 
        type=str, 
        default='figures/MCR_analysis',
        help='Directory to save output figures (default: figures/MCR_analysis)'
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None,
        help='Custom feature pairs in format "feat1,feat2;feat3,feat4" (default: use predefined TCR pairs)'
    )
...
def main():
    ...
    # Validate input file
    if not os.path.exists(args.mcr_path):
        print(f"❌ Error: MCR data file not found: {args.mcr_path}")
        sys.exit(1)
    ...
    if args.features:
        try:
            selected_features = parse_feature_pairs(args.features)
            print(f"📋 Using custom feature pairs: {selected_features}")
        except ValueError as e:
            print(f"❌ Error parsing feature pairs: {e}")
            sys.exit(1)
    ...
    try:
        ...
        if args.output_results and results:
            ...
            all_results_list = []
            for (feature1, feature2), df in results.items():
                df_copy = df.copy()
                df_copy['Feature1'] = feature1
                df_copy['Feature2'] = feature2
            ...
            if all_results_list:
                ...
                # Reorder columns
                cols = ['Feature1', 'Feature2', 'Sample1', 'Sample2', 'P_Value_Raw']
